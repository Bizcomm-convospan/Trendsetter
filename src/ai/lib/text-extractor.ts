'use server';
/**
 * @fileOverview An AI-powered text extractor.
 */
...
export async function extractTextFromHtml(html: string): Promise<string> {
  ...
  try {
    ...
    // Fallback to returning an empty string or a snippet of the raw text
    // to avoid complete failure of the parent flow.
    return '';
  }
}
