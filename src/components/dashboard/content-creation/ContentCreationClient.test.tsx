...
jest.mock('@/app/actions', () => ({
  ...
  handleDiscoverTrends: jest.fn(), // Mock the new dependency
}));
...
describe('ContentCreationClient', () => {
  beforeEach(() => {
    ...
    mockedHandleDiscoverTrends.mockResolvedValue({ data: { discoveredTrends: [] } });
  });

  it('renders the component and generation form correctly', () => {
    render(<ContentCreationClient />);

    expect(screen.getByText('Step 1: Discover Trending Topics')).toBeInTheDocument();
    expect(screen.getByText('Step 2: Generate a New Article')).toBeInTheDocument();
  ...
  it('displays draft articles from Firestore', async () => {
    ...
    mockedOnSnapshot.mockImplementation((query, callback) => {
      const queryStr = JSON.stringify(query); // Simple way to differentiate queries
      if (queryStr.includes("draft")) {
