...
describe('TrendDiscoveryClient', () => {
  ...
  it('calls onSelectTrend when a trend article generation is requested', async () => {
    const user = userEvent.setup();
    const onSelectTrendMock = jest.fn();
    mockedHandleDiscoverTrends.mockResolvedValue({
      data: {
        discoveredTrends: [
          {
    ...
    render(<TrendDiscoveryClient onSelectTrend={onSelectTrendMock} />);
    const submitButton = screen.getByRole('button', { name: /Discover Trends/i });
    await user.click(submitButton);
    ...
    expect(onSelectTrendMock).toHaveBeenCalledWith('Sustainable Energy');
  });
});
